...
// Constructor
// This initializes the GUI Window, Placing things like the BG, BAR, Hide
// button, close button, and title in the correct place.
SFGUI::SFMLGUI::SFMLGUI(sf::RenderWindow &window, std::string window_title)
    : pos(DEFAULT_GUI_POSITION), dragging(false), resizing(false),
  ...
  BAR.setPosition(sf::Vector2f(DEFAULT_GUI_POSITION.x,
                               (DEFAULT_GUI_POSITION.y - DEFAULT_BAR_HEIGHT)));
  ...
  HIDE_BUTTON.setOrigin(sf::Vector2f(HIDE_BUTTON.getSize().x / 2.f,
                                     HIDE_BUTTON.getSize().y / 2.f));
...
void SFGUI::SFMLGUI::setWindowTitle(const std::string &text) {
  ...
  WIN_TEXT.setPosition(sf::Vector2f(
      BAR.getPosition().x + (WIDGET_PADDING_HOR * 2) + HIDE_BUTTON.getSize().x,
      barBounds.position.y + (barBounds.size.y - textBounds.size.y) / 2.f -
...
void SFGUI::SFMLGUI::Draw() {
  if (!closed) {
    if (!hidden) {
      SF_WINDOW->draw(BG);
      // SCISSOR TEST for masking widgets, keeping them from overflowing outside
      // the gui. //
      ...
      glScissor(pos.x, SF_WINDOW->getSize().y - (pos.y + size.y), size.x,
                size.y);
      for (SFWIDGET *widget : SF_WIDGETS) {
        widget->Draw(*SF_WINDOW);
      }
      glScissor(guiPrevScissor[0], guiPrevScissor[1], guiPrevScissor[2],
                guiPrevScissor[3]);
    ...
    SF_WINDOW->draw(BAR);
    {
      ...
      GLint scissorY = static_cast<GLint>(
          SF_WINDOW->getSize().y - (BAR.getPosition().y + BAR.getSize().y));
      GLint scissorWidth = static_cast<GLint>(CLOSE_BUTTON_BG.getPosition().x -
                                              BAR.getPosition().x);
  ...
  if (!closed) {
    ...
    if (isMouseInsideRect(*SF_WINDOW, RESIZE_AREA)) {
      ...
      if (leftMousePressed) {
        ...
        if (newWidth < MINIMUM_GUI_SIZE.x)
          newWidth = MINIMUM_GUI_SIZE.x;
        if (newHeight < MINIMUM_GUI_SIZE.y)
          newHeight = MINIMUM_GUI_SIZE.y;

        BG.setSize(sf::Vector2f(newWidth, newHeight));
    ...
    if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) {
      ...
      if (resizing) {
        ...
        HIDE_BUTTON_BG.setPosition(BAR.getPosition());
        HIDE_BUTTON.setPosition(HIDE_BUTTON_BG.getPosition() +
                                sf::Vector2f(HIDE_BUTTON_BG.getSize().x / 2,
                                             HIDE_BUTTON_BG.getSize().y / 2));
        CLOSE_BUTTON_BG.setPosition(sf::Vector2f(
            BAR.getPosition().x + BAR.getSize().x - CLOSE_BUTTON_BG.getSize().x,
            BAR.getPosition().y));
        CLOSE_BUTTON.setPosition(CLOSE_BUTTON_BG.getPosition() +
                                 sf::Vector2f(CLOSE_BUTTON_BG.getSize().x / 2,
                                              CLOSE_BUTTON_BG.getSize().y / 2));
        RESIZE_RECT.setPosition(sf::Vector2f(
            BG.getPosition().x + BG.getSize().x - RESIZE_RECT_SIZE,
            BG.getPosition().y + BG.getSize().y - RESIZE_RECT_SIZE));
        RESIZE_AREA.setPosition(
            sf::Vector2f(BG.getPosition().x + BG.getSize().x,
                         BG.getPosition().y + BG.getSize().y));
      }
        ...
        if (!hidden) {
          for (int i = 0; i < SF_WIDGETS.size(); i++) {
            ...
            if (SF_WIDGETS[i]->w_index == 0) {
              ...
              xPos = WIDGET_PADDING_HOR + pos.x;
              yPos = SF_WIDGETS[i - 1]->w_pos.y +
                     SF_WIDGETS[i - 1]->w_size.size.y + WIDGET_PADDING_VER;
            }
...
void SFGUI::SFMLGUI::Close() { closed = true; }

void SFGUI::SFMLGUI::Open() { closed = false; }
...
void SFGUI::SFMLGUI::Setup() {
  for (int i = 0; i < SF_WIDGETS.size(); i++) {
    ...
    if (SF_WIDGETS[i]->w_index == 0) {
      ...
      xPos = WIDGET_PADDING_HOR + pos.x;
      yPos = SF_WIDGETS[i - 1]->w_pos.y + SF_WIDGETS[i - 1]->w_size.size.y +
             WIDGET_PADDING_VER;
    }
    sf::Vector2f finalPosition = sf::Vector2f(xPos, yPos);
