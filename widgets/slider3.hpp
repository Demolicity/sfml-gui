...
class Slider3 : public SFWIDGET
{
    public:
        ...
        //takes in a struct pointer and extracts the xyz values, where all 3 sliders have same min/max values with default width.
         template <typename StructT>
        Slider3 (StructT *structPtr, const std::string& text) : SFWIDGET("Slider3"), label(TEXT_FONT),
        slider(&structPtr->x, text, 0, 1, DEFAULT_BUTTON_SIZE.x),
        slider2(&structPtr->y, text, 0, 1, DEFAULT_BUTTON_SIZE.x),
        slider3(&structPtr->z, text, 0, 1, DEFAULT_BUTTON_SIZE.x)
        {
            label_text = text;
            this->SetupSlider();
        }
        ...
        void setWindow(sf::RenderWindow* window) override {
            ...
            slider3.setWindow_and_GuiRef(window, guiRef);
        }
